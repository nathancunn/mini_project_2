%%Code to handle the negative binomial data type.
%%This data type treats each feature as being drawn from a
%negative binomial distribution, and forms the overal marginal likelihood
%by taking the product of the distributions for each feature
%%
%%Code written by Rich Savage.
%%
function [output1 output2] = NegativeBinomial(input, mode, geneIndex, currentData)
switch mode
 case 'init'
  ...
  %HYPERPARAMETERS
  [clusterStruct.a] = deal(0.5);%%Jeffreys' value
  [clusterStruct.b] = deal(0.5);%%JEffreys' value
  [clusterStruct.r] = deal(1);
  ...
  for i = 1:maxNumberOfComponents
    ...
    currentCluster.sumLogGammaPlusR  = sum(gammaln(currentCluster.r+dataInCluster(indices,:)), 1);
    currentCluster.logicalGeneIDs    = logicalIndices;
    if (nGenesInCluster > 0)
      currentCluster  = NegativeBinomial(currentCluster, 'marginal');
    end
 ...
 case 'marginal'
  ...
  sumLogGammaPlusR = input.sumLogGammaPlusR;
  nGenes           = input.nGenes;
  a                = input.a;
  b                = input.b;
  r                = input.r;
  ...
  logEv = gammaln(sumCounts + r);
  logEv = logEv - gammaln(sumCounts + nGenes*r + a + b); 
  logEv = logEv + sumLogGammaPlusR;
  logEv = logEv - sumLogGamma;
  logEv = logEv - nGenes*gammaln(r);
  logEv = logEv + gammaln(nGenes*r + b);  
  logEv = logEv + gammaln(a+b);
  logEv = logEv - gammaln(a);
  logEv = logEv - gammaln(b);
...
%  end
 ...
 case 'removeGene'
  ...
  if (nGenesInCluster == 0)
    ...
    output1 = NegativeBinomial(output1, 'marginal');
  end
 ...
 case 'featureSelection'
  ...
  for i=1:nClusters
    ...
    [dum, newLogEvRatio] = NegativeBinomial(input(currentIndex), 'marginal');
  ...
  %%FOR NOW, DISABLE THE FEATURE SELECTION
  disp('Negative Binomial feature selection currently disabled...')
  newFeatureParameters(:) = 1;

    ...
    for m = 1:numbofparts
        for k = 1:K;
            ...
            if(n == 1)
                ...
                occupiedClusters = unique(s(1:(n - 1),k));
                for ind1 = 1:length(occupiedClusters)
                    ind = occupiedClusters(ind1);
                    % logprob(1, ind) = sum(- 0.5 * (dataForThisContext(i, :) - proposedClustersForThisContext(ind).empiricalMeans).^2 ./ (0.5 * 1 + proposedClustersForThisContext(ind).squaredResiduals) - 0.5 * log(0.5 * 1 + proposedClustersForThisContext(ind).squaredResiduals));
                    ...
                    if(proposedClustersForThisContext(ind).nGenes > 1)
                        sigmaInverse = pinv(cov(dataForThisContext(proposedClustersForThisContext(ind).logicalGeneIDs, :)));
                        sigmaInverse = sigmaInverse ./ proposedClustersForThisContext(ind).nGenes / 0.5 + (1 / (1 - 0.5));
                        logprob(1, ind) = - 0.5 * (meanCentred * sigmaInverse * meanCentred.');
                    else
                        logprob(1, ind) = - 0.5 * (meanCentred * (eye(size(dataForThisContext, 2)) * 0.5) * meanCentred.');
                    end
                   
                end
                unoccupiedClusters = setdiff(1:N,occupiedClusters);
                logprob(1, unoccupiedClusters) = - 0.5 * (dataForThisContext(i, :) * (eye(size(dataForThisContext, 2)) * 0.5) * dataForThisContext(i, :).');
                % logprob(1, unoccupiedClusters) = sum(- 0.5 * (dataForThisContext(i, :)).^2 ./ (0.5 * 1) - 0.5 * log(0.5 * 1));
                %logprob(1, unoccupiedClusters) = mvnpdf(dataForThisContext(i, :), 0, repmat(1, 1, size(dataForThisContext, 2)));
